...
def load_medicine_db():
    try:
        ...
        st.warning("⚠️ 'Medicine_description.xlsx' not found or is formatted incorrectly.")
...
# =========================================================
# DETACHED NLP SYMPTOM VECTOR COUPLING ENGINE (UPGRADED)
# =========================================================
def encode_symptoms_to_dict(text, feature_list, vital_features):
    text = text.lower().strip()
    
    # Expanded dictionary to catch how real people type symptoms
    symptom_map = {
        "fever": ["fever", "temperature", "hot", "chills", "sweats"],
        "cough": ["cough", "coughing", "hacking", "phlegm"],
        "headache": ["headache", "migraine", "head pounding", "head hurt"],
        "chest_pain": ["chest pain", "tight chest", "heart pain", "heaviness"],
        "shortness_of_breath": ["breath", "breathing", "wheezing", "gasping"],
        "rash": ["rash", "skin", "itch", "redness", "hives"],
        "fatigue": ["fatigue", "weak", "tired", "exhausted", "lethargic"],
        "vomiting": ["vomiting", "nausea", "throw up", "puking", "sick to stomach"],
        "dizziness": ["dizzy", "lightheaded", "spinning", "faint", "vertigo"],
    ...
    for feature in feature_list:
        ...
        # Method 1: Check for exact feature string in text
        if clean_feature in text:
            found = 1
            
        # Method 2: Check our expanded synonym map
        elif feature in symptom_map:
            if any(keyword in text for keyword in symptom_map[feature]):
                found = 1
                
        # Method 3: Smart multi-word matching
        else:
            feature_words = [w for w in clean_feature.split() if len(w) > 3]
            if len(feature_words) > 1 and all(w in text for w in feature_words):
                found = 1
...
if st.button("🚀 Execute Hybrid Pipeline Inference"):
    ...
    # Saving pipeline dictionary outputs to Session State
    st.session_state.results = {
...
if st.session_state.diagnosis_triggered and "status_ui" in st.session_state.results:
    ...
    with tab3:
        ...
        # 1. Extract base prediction words (ignoring small words)
        base_words = [w.lower() for w in res["clinical_prediction"].split() if len(w) > 3]
        
        # 2. Synonym Bridge: Maps system predictions to common database keywords
        synonym_bridge = {
            "cardiac": ["heart", "angina", "cardio", "pressure", "hypertension"],
            "diabetes": ["sugar", "diabet", "glucose", "insulin"],
            "fever": ["fever", "pyrexia", "temperature", "paracetamol", "infection"],
            "respiratory": ["breath", "asthma", "lung", "cough", "bronch"],
            "infection": ["bacteria", "infect", "antibiotic", "virus"]
        }
        
        # Build a robust search pool
        search_terms = set(base_words)
        for word in base_words:
            for key, synonyms in synonym_bridge.items():
                if key in word:
                    search_terms.update(synonyms)
        
        # 3. Scan the database using the expanded search pool
        mask = med_db["Reason"].astype(str).str.lower().apply(
        ...
        if not matched_meds.empty:
            for _, row in matched_meds.head(5).iterrows():
                st.markdown(f"""
                <div class='med-card'>
                    <b>Generic Formulation Compound: {row.get('Drug_Name', 'Unknown')}</b><br>
                    <small>Indicated Context Framework: {row.get('Reason', 'N/A')}</small><br>
                    <p style='margin-top:5px;'>Description Details: {row.get('Description', 'No description.')}</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info(f"No explicit medicine records matching '{res['clinical_prediction']}' (or related keywords) inside local database storage maps.")
...
Generates ML Hypothesis Output
            ...
            If a patient matches critical emergency criteria (e.g., SpO2 < 90% or acute chest pain), the symbolic engine bypasses the ML probability matrix and enforces a higher risk alert status.
