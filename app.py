...
def send_to_doctor(receiver_email, report_data, meds):
    ...
    DIAGNOSIS SUMMARY:
    - Vitals Diagnosis: {report_data['v_diag']}
    - Symptom Detection: {report_data['s_diag']}
    - Triage Status: {report_data['urgency']}
    
    RECOMMENDED THERAPY:
    {med_text if meds else "No specific medications suggested."}
    
    This is an automated alert generated by the Clinical AI Portal.
    """
...
def get_clinical_triage(urgency_status):
    st.subheader("🛑 Clinical Triage & Action Plan")
    if urgency_status == "EMERGENCY":
        st.error("🚨 **CRITICAL ALERT:** Immediate clinical intervention required. Visit ER.")
    else:
        st.success("✅ **STABLE:** Routine follow-up recommended. Monitor for 48 hours.")
...
with col_r:
    ...
    s_input = st.text_area("Enter Symptoms")
    doc_email = st.text_input("Doctor Email Address (to send alert)")
...
if st.button("RUN FULL DIAGNOSTIC", type="primary"):
    if model_assets[0] is not None:
        ...
        matched_scores = {d: (10 if d.lower() in user_text else 0) + sum(1 for sym in s_set if sym in user_text) for d, s_set in disease_map.items()}
